...
Change Log:
7/21/11	Rory:	- Created
3/5/2012 Rory:	- Fixed bug in the delete function which was inproperly searching for the text string in the whole field of the query string variable and not for just the name. This was resulting
				  in it getting false positives like ''name=humidifier' and when searching for ID, it would be found in humidifier 
*/
	...
	*/
		...
		hint="checks the given masks and determines if the variables passed in match"
	{
		...
		for(var i=1;i LTE arrayLen(variables.masks); i=i+1)
		{
			...
			var maskArray = getMaskArray(variables.masks[i]);
			...
			if(variables.maskId IS NOT 0)
			{
				...
				variables.maskArray = getMaskArray(variables.masks[variables.maskid]);
	...
	{
		writeDump(variables.queryStringStruct);
		var out = "";
		for(var i=1; i LTE arrayLen(variables.queryStringStruct); i = i+1)
		{
	...
	private function getMaskArray(required mask)
	{
