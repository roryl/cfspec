<cfcomponent displayname="ConfigManager" hint="Controlls data retrieved from mxunit-confg.xml" output="false">
  ...
  <cffunction name="ConfigManager" access="public" output="true" returntype="ConfigManager" hint="Constructor" cachedwithin="#createTimeSpan(0, 0, 0, 10)#">
 ...
 <cffunction name="getConfigElement" cachedwithin="#createTimeSpan(0, 0, 0, 10)#" access="remote" output="false" returntype="array" hint="Given a type and name, returns the first matching XML element node.">
  ...
  <!---
   ...
   --->
  <cffunction name="getConfigElementValue" cachedwithin="#createTimeSpan(0, 0, 0, 10)#" access="remote" output="false" returntype="String" hint="Given a type and name, returns the first matching XML element value.">
    <cfargument name="type" type="string" required="true" />
    <cfargument name="name" type="string" required="true" />
    <cfset var element = xmlSearch(this.configXml,"/mxunit-config/config-element[@type='#arguments.type#' and @name='#arguments.name#']") />
  ...
  <cffunction name="getConfigElements" cachedwithin="#createTimeSpan(0, 0, 0, 10)#" access="remote" output="false" returntype="array" hint="Given an XPath expression, return an array of matching config elements">
  ...
  <cffunction name="getConfigElementAttributeCollection"  cachedwithin="#createTimeSpan(0, 0, 0, 10)#" access="remote" output="false" returntype="any" hint="Given a type and name, returns all of the attributes in a struct, omitting the type and name.">
    <cfargument name="type" type="string" required="true" />
    <cfargument name="name" type="string" required="true" />
    <cfset var element = xmlSearch(this.configXml,"/mxunit-config/config-element[@type='#arguments.type#' and @name='#arguments.name#']") />
